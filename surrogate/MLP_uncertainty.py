...
class MLP_uncertainty(nn.Module):
    def __init__(self, predict_lod=True):
        ...
        self.model_filename = "surrogate/mlp_state_dict_uncert_lod.pth" if self.predict_lod else "surrogate/mlp_state_dict_uncert_cdl.pth"
        out_dim=1 if self.predict_lod else 2
        in_dim=1 if self.predict_lod else 2
        self.layers = nn.Sequential(
                nn.Linear(54+in_dim, 128),
                nn.LeakyReLU(),
                nn.Linear(128, 256),
                nn.LeakyReLU(),
                nn.Linear(256, 256),
    ...
    def forward(self, x, y=None):
        if y is None:
            y = torch.zeros_like(x)[...,:1] if self.predict_lod else torch.zeros_like(x)[...,:2]
        ...
        for epoch in range(NEPOCHS):
            ...
            for bat in range(dataT.batchN):
                ...
                if self.predict_lod:
                    ...
                    loss = (loss_fn(y, cdlT) * weights).mean()
                    
                y = self(targetT, (cdlT[:,1]/cdlT[:,0]).unsqueeze(-1)) if self.predict_lod else self(targetT, cdlT)
                if self.predict_lod:
                    loss += loss_fn(y[:,0], cdlT[:,1]/cdlT[:,0])
                else:
                    loss += (loss_fn(y, cdlT) * weights).mean()
